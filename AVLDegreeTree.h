    ...
    */
       ...
       */
        avl_Node *RR_Rotation(avl_Node *node) {
            ...
            if (temp->left != NULL) {
                ...
                if(node->left!=NULL) {
                    ...
                    node->sum = temp->left->sum + node->left->sum +
                                temp->left->data + node->left->data;
                }
                else {
                    node->degree = temp->left->degree + 0 + 1;
                    node->sum = temp->left->sum + temp->left->data;
                }
            }
            else{
                if(node->left!=NULL) {
            ...
            if(temp->right != NULL)
                temp->sum = node->sum + node->data + temp->right->sum + temp->right->data;
            else
                temp->sum = node->sum + node->data;
            node->parent = temp;
      ...
      */
        avl_Node *LL_Rotation(avl_Node *node) {
            ...
            if (temp->right != NULL) {
                ...
                if(node->right!=NULL) {
                    ...
                    node->sum = temp->right->sum + node->right->sum +
                                temp->right->data + node->right->data;
                }
                else {
                    node->degree = temp->right->degree + 0 + 1;
                    node->sum = temp->right->sum + temp->right->data;
                }
            }
            else{
                if(node->right!=NULL) {
            ...
            temp->degree = temp_degree;
            if(temp->left != NULL)
                temp->sum = node->sum + node->data + temp->left->sum + temp->left->data;
            else
                temp->sum = node->sum + node->data;
            node->height = std::max(height(node->left), height(node->right)) + 1;
        ...
        */
        ...
        avl_Node *insert(avl_Node *node, T data, Compare f) {
            ...
            int counter = 0;
            int data_inserted = inserted->data;
            bool flag = false;
    ...
    //    T* postorder(avl_Node *node);
       ...
       /* static avl_Node* copyTree(avl_Node* node){
        ...
        int Kth_biggest_sum(avl_Node* node, int k, Action action,Function f){
            ...
            if(node->right != NULL){
                sum+= action(node->right->sum,node->right->data);
            }
            return action(sum,node->data);
        }
    ...
    public:
       ...
       */
        ...
        void operator=(const AVLDegreeTree<T>& toCopy){
            //destroyTree(root);
       ...
       */
        ...
        int mergeTrees(AVLDegreeTree& tree,Compare f,Pred g,doSomething d,addOn a){
